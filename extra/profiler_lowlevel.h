...
typedef signed long long Profiling_Int64;
...
struct Profiling {
    ...
    static Program_Zone *zone_stack[MAX_PROFILING_STACK_DEPTH];
...
// Thanks to Paul Du Bois for the os_fast_get_integer_timestamp simplification.
// inline Profiling_Int64 __declspec(naked) __stdcall os_fast_get_integer_timestamp() {
//     __asm {
//         rdtsc;
//         ret;
//     }
...
inline Profiling_Int64 os_fast_get_integer_timestamp() { return __rdtsc(); }

#ifdef PROFILE_HOOKS

inline void Profiling::Enter_Zone(Program_Zone &zone) {
    Program_Zone *old_zone = zone_stack[stack_pos];
    zone_stack[++stack_pos] = &zone;

    // zone.t_self_start = os_fast_get_integer_timestamp();
...
inline void Profiling::Exit_Zone() {
    Program_Zone *zone = zone_stack[stack_pos--];

    Profiling_Int64 t_end = os_fast_get_integer_timestamp();

    // zone->total_self_ticks += (t_end - zone->t_self_start);
    zone->total_hier_ticks += (t_end - zone->t_hier_start);

    // Program_Zone *new_zone = zone_stack[stack_pos];
